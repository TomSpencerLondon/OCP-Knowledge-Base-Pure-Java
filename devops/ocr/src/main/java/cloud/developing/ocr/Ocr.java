...
public class Ocr {
	...
	private static final Function<DetectTextResponse, String> DEFAULT_STRATEGY = response -> response.hasTextDetections()
	        ? response.textDetections().stream().filter(td -> td.parentId() == null).map(td -> td.detectedText()).collect(
	                joining(" "))
	        : BLANK;

	static {
		ANALYSIS_STRATEGIES.put(DEFAULT, DEFAULT_STRATEGY);
		ANALYSIS_STRATEGIES.put(BIG_BOX, response -> {
			Optional<TextDetection> biggestBoxOptional = response.hasTextDetections()
			        ? response.textDetections().stream().sorted(areaComparator).findFirst()
			        : empty();

			var text = BLANK;
			if (biggestBoxOptional.isPresent()) {
				var biggestBox = biggestBoxOptional.get();
				var y = round(getTop(biggestBox));
				text = response.textDetections().stream()
	...
	public OcrOutput recognize(OcrInput ocrInput) {
		var response = rekognition.detectText(DetectTextRequest.builder()
		        .image(i -> i.s3Object(o -> o.bucket(ocrInput.bucket()).name(ocrInput.key()))).build());
		var text = ANALYSIS_STRATEGIES.getOrDefault(ocrInput.analysisStrategy(), DEFAULT_STRATEGY).apply(response);
  ...
  public static void main(String[] args) {
    var result = new Ocr()
        .recognize(
            new OcrInput("tomspencer-license-plates",
                "vehicle-1.jpg",
                BIG_BOX));
