...
<<<<<<< HEAD
  // const r = data.ladder.challenges_accepted.filter(x => x.challenger.id === auth?.user?.id || x.opponent.id === auth?.user?.id ).map(x => '1 ')
  const r = data.ladder.challenges_accepted.filter(x => x.challenger.id == auth?.user?.id || x.opponent.id == auth?.user?.id).map(x => '1 ')
  console.log('NUM ACCEPTED', r.length)
  console.log('ACCEPTED', data.ladder.challenges_accepted, auth?.user?.id)
  const p = data.ladder.challenges_pending.filter(x => x.challenger.id == auth?.user?.id || x.opponent.id == auth?.user?.id).map(x => '1 ')
  console.log('NUM PENDING', p.length)
...
<<<<<<< HEAD
  function resultStatus(userId) {
    const rank = data.ladder.ranks.find(r => r.user.id == userId)
    if (!rank) return null
    if (rank.user.away) { // On vaction?
      return ' ⛱️'
    }
    const lastResult = data.ladder.results.find(r => r.challenger.id == userId || r.opponent.id == userId)
    if (!lastResult) return null;
    if (lastResult.winner.id == userId
      && (rank.user.rank_histories.length > 0 && rank.rank < rank.user.rank_histories[rank.user.rank_histories.length - 1].rank)) { // Did they win & move up / defend?
      return '  ⬆️ '
    } else if (lastResult.loser.id == userId
      && (rank.user.rank_histories.length > 0 && rank.rank > rank.user.rank_histories[rank.user.rank_histories.length - 1].rank)) { // Did they win & move up / defend?
      return '  ⬇️ '
    }
  }
...
<<<<<<< HEAD
                ...
                &nbsp;
                👥 &nbsp;{countAccepted(v.user.id)}
                &nbsp;
                ✉️ &nbsp;{countPending(v.user.id)} 
=======
                {starPlayerId == v.user.id ? '  ⭐  ' : null}
                {v.user.away ? ' ⛱ ️' : null}
                {hotStreak(v.rank, v.user.rank_histories)}
